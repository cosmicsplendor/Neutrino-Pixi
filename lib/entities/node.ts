...
const NodeMixin = function<TBase extends Constructor<Container>>(Base: TBase) {
...
const SpriteMixin = <TBase extends Constructor<Sprite>>(Base: TBase) => {
    return class NodeClass extends Base {
        frameZ = "abc"
    }
}

const NodeClass = NodeMixin(Container)
const SpriteClass = SpriteMixin(NodeMixin((Sprite)))
