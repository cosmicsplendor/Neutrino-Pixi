...
export const fixedAabb = (b1: RectBounds, b2: RectBounds) => {
    if (
...
export const aabb = (b1: RectBounds, b2: RectBounds) => {
    if (
        b1.x + b1.width <= b2.x || 
        b1.x >= b2.x + b2.width ||
        b1.y + b1.height <= b2.y ||
...
export const circCirc = (b1: CircBounds, b2: CircBounds) => {
...
export const aabbCirc = (rectBounds: RectBounds, circBounds: CircBounds) => {
...
export const circPlank = (circBounds: CircBounds, plankBounds: PlankBounds) => {
...
export const contains: (param: { box: RectBounds, point: Coords }) => boolean = ({ box, point }) => {
